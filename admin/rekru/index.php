...
<head>
    ...
    <link rel="stylesheet" href="rekru.css">
        ...
        <div class="title">
            ...
            <div class="down">
                ...
                <?php
                    ...
                    if(isset($_POST['ASC'])){
                        $asd = $_POST['sort'];
                        $que = "SELECT * FROM rekru ORDER BY " . $asd . "";
                        $result = query($que);
                   ...
                   elseif(isset($_POST['DESC'])){
                        $asd = $_POST['sort'];
                        $que = "SELECT * FROM rekru ORDER BY " . $asd . " DESC";
                        $result = query($que);  
                        while ($row = mysqli_fetch_assoc($result) ){
                           results($row);
                   }}
                   else{
                        $que = "SELECT * FROM rekru";
                        $result = query($que);         
                        while ($row = mysqli_fetch_assoc($result) ){
                           results($row);
                   }}
                    if(isset($_POST['nie'] ) ) {
                        $qe = "DELETE FROM `rekru` WHERE `rekru`.`id` = " . $_POST['id'] . "";
                        $result1 = query($qe);
                    }
                 ?>
            </div>
