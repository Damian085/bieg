    ...
    <body>
            <div class="up">
                ...
                <select name="sort">
                    ...
                    <button name="ASC">Rosnąco</button>
                    <button name="DESC">Malejąco</button>
                </form>
            </div>
            <div class="down">
                <table>
                ...
                <?php
                        ...
                        <tr>
                            ...
                            <td class='accept'>
                                <form action='index.php' method='POST'>
                                    <button class='tak' name='tak'>Tak</button>
                                    <input type='hidden' name='id' value=" . $row['id'] . ">
                                    <button class='nie'name='nie'>Nie</button>
                                </form>
                            </td>
                        </tr>";
                    }
                    if(isset($_POST['ASC'])){
                        $asd = $_POST['sort'];
                        $que = "SELECT * FROM zawodnicy WHERE accept = 0 ORDER BY " . $asd . "";
                        $result = query($que);
                   ...
                   elseif(isset($_POST['DESC'])){
                        $asd = $_POST['sort'];
                        $que = "SELECT * FROM zawodnicy WHERE accept = 0 ORDER BY " . $asd . " DESC";
                        $result = query($que);  
                        while ($row = mysqli_fetch_assoc($result) ){
                           results($row);
                   }}
                   else{
                        $que = "SELECT * FROM zawodnicy WHERE accept = 0";
                        $result = query($que);         
                        while ($row = mysqli_fetch_assoc($result) ){
                           results($row);
                   }}
                    if (isset($_POST['tak'] ) ) {
                        $qe = "UPDATE `zawodnicy` SET `accept` = '1' WHERE `zawodnicy`.`id` = " . $_POST['id'] . "";
                        $result1 = query($qe);
